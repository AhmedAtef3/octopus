...
then
	...
	echo "| -Columns: Name of each column followed by it's datatype (INT,TEXT) and optional PRIMARY_KEY constrain."
	...
	echo "|Arguments: "
	echo "| -TableName: Should be an existing table."
	echo "| -Record: Column values separated by spaces."
	echo "|"
	echo "+${bold}DeleteRecord${normal}-Deletes all records that matches a value from table."
	echo "|Arguments: "
	echo "| -TableName: Should be an existing table."
	echo "| -Conditions: The conditions of the records represented in the following syntax separated by spaces : COLUMN=VALUE."
	echo "|"
